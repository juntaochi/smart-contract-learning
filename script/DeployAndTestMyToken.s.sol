// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "forge-std/Script.sol";
import "../contracts/MyToken.sol";

/**
 * Script to deploy MyToken and create test transfers for indexer testing
 */
contract DeployAndTestMyToken is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        address deployer = vm.addr(deployerPrivateKey);
        
        console.log("Deploying with account:", deployer);
        
        vm.startBroadcast(deployerPrivateKey);
        
        // Deploy MyToken
        MyToken token = new MyToken("Test Token", "TEST");
        console.log("MyToken deployed at:", address(token));
        console.log("Total supply:", token.totalSupply());
        console.log("Deployer balance:", token.balanceOf(deployer));
        
        // Create some test accounts
        address alice = address(0x1111111111111111111111111111111111111111);
        address bob = address(0x2222222222222222222222222222222222222222);
        address charlie = address(0x3333333333333333333333333333333333333333);
        
        // Transfer to test accounts
        console.log("\n--- Creating test transfers ---");
        
        token.transfer(alice, 1000 * 10**18);
        console.log("Transferred 1000 TEST to Alice");
        
        token.transfer(bob, 2000 * 10**18);
        console.log("Transferred 2000 TEST to Bob");
        
        token.transfer(charlie, 3000 * 10**18);
        console.log("Transferred 3000 TEST to Charlie");
        
        token.transfer(alice, 500 * 10**18);
        console.log("Transferred 500 TEST to Alice (again)");
        
        token.transfer(bob, 750 * 10**18);
        console.log("Transferred 750 TEST to Bob (again)");
        
        vm.stopBroadcast();
        
        console.log("\n--- Deployment Summary ---");
        console.log("Token Address:", address(token));
        console.log("Alice:", alice);
        console.log("Bob:", bob);
        console.log("Charlie:", charlie);
        console.log("\nUpdate your backend/.env file:");
        console.log("TOKEN_ADDRESSES=\"%s\"", address(token));
    }
}
